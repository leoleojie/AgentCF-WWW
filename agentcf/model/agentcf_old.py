...
class AgentCF(SequentialRecommender):
        ...
        rec_context = \
            {'agent_type' :'recagent',
             ...
             'llm' :{'model' :self.config['llm_model'] ,'llm_type' :self.config['llm_model'], 'temperature' :self.config['llm_temperature'] ,'max_tokens' :self.config['max_tokens']
                     ,'api_key_list' :self.config['api_key_list'] ,'current_key_idx': self.config['current_key_idx'], },
             'llm_chat' :{'model' :'gpt-3.5-turbo-16k-0613' ,'llm_type' :'gpt-3.5-turbo-16k-0613'
                          ,'temperature' :self.config['llm_temperature_test']
                          ,'max_tokens' :self.config['max_tokens_chat'] ,'api_key_list' :self.config['api_key_list']
                          ,'current_key_idx': self.config['current_key_idx'] ,},
    ...
    def load_user_context(self):
        user_context = {}
        user_context[0] = {'agent_type' :'useragent', 'role_description' :{'age': '[PAD]', 'user_gender': '[PAD]' ,'user_occupation' :'[PAD]'}
                           ,'memory_1' :['[PAD]'] ,'update_memory' :['[PAD]'],
                           ...
                           'llm' :{'model' :self.config['llm_model'] ,'llm_type' :self.config['llm_model']
                                   ...
                                   ,'max_tokens' :self.config['max_tokens'] ,'api_key_list' :self.config['api_key_list']
                                   ,'current_key_idx': self.config['current_key_idx']},
                           'llm_chat' :{'model' :'gpt-3.5-turbo-16k-0613' ,'llm_type' :'gpt-3.5-turbo-16k-0613'
                                        ,'temperature' :self.config['llm_temperature']
                ...
                for line in file:
                    ...
                    user_context[self.user_token_id[user_id]] = \
                        {'agent_type' :'useragent',
                         'role_description': {'age': user_age, 'user_gender': user_gender
    ...
    def load_item_context(self):
        item_context = {}
        item_context[0] = {'agent_type' :'itemagent', 'role_description' :{'item_title': '[PAD]', 'item_release_year': '[PAD]'
                                                ,'item_class' :'[PAD]'} ,'memory' :['[PAD]'] ,'memory_embedding' :{}
                           ,'update_memory' :['[PAD]'], 'item_prompt_template_true': self.config['item_prompt_template_true'],
    ...
    def generate_embedding(self, embedding_contents):
        ...
        for i in range(0, batch_size, self.api_batch):
            embeddings += asyncio.run(
                self.embedding_agent.llm.agenerate_response(embedding_contents[i:i + self.api_batch]))
    ...
    def forward(self, batch_user, batch_pos_item, batch_neg_item):
        ...
        system_forward_prompts = [
            self.rec_agent.astep_forward(int(batch_user[i]), user_descriptions[i], pos_item_descriptions[i],
                                         neg_item_descriptions[i]) for i in range(batch_size)]
        system_responses = []
        for i in range(0, batch_size, self.api_batch):
    ...
    def backward(self, system_reasons, batch_user, batch_pos_item, batch_neg_item):
        ...
        user_backward_prompts = [
            self.user_agents[int(batch_user[i])].astep_backward(system_reasons[i], pos_item_titles[i],
                                                                neg_item_titles[i], pos_item_descriptions_forward[i],
                                                                neg_item_descriptions_forward[i]) for i in
            range(batch_size)]
        ...
        user_update_descriptions = [
            self.user_agents[0].output_parser.parse_update(response["choices"][0]["message"]["content"]) for response in
            user_update_descriptions]
        for i, user in enumerate(batch_user):
        ...
        self.logging_during_updation(batch_user, system_reasons, user_backward_prompts, pos_item_descriptions_forward,
                                     neg_item_descriptions_forward, user_update_descriptions, item_update_memories)
    ...
    def backward_true(self, system_reasons, batch_user, batch_pos_item, batch_neg_item, round_1):
        ...
        if round_1:
            user_backward_prompts = [
                self.user_agents[int(batch_user[i])].astep_backward_true(system_reasons[i], pos_item_titles[i],
                                                                         neg_item_titles[i],
            ...
            user_update_descriptions = [
                self.user_agents[0].output_parser.parse_update(response["choices"][0]["message"]["content"]) for
                response in user_update_descriptions]
            for i, user in enumerate(batch_user):
                self.user_agents[int(user)].update_memory.append(user_update_descriptions[i])
    ...
    def calculate_loss(self, interaction):
        ...
        for i, user in enumerate(batch_user):
            ...
            user_forward_description[i], self.item_text[int(batch_pos_item[i])], self.item_text[int(batch_neg_item[i])],
            pos_item_forward_description[i], neg_item_forward_description[i], accuracy[i], system_reasons[i])] = \
    ...
    def logging_during_updation(self, batch_user, system_explanations, user_backward_prompts,
                                pos_item_descriptions_forward, neg_item_descriptions_forward, user_update_descriptions,
                                item_update_memories):
        batch_size = len(batch_user)
        for i in range(batch_size):
    ...
    def full_sort_predict(self, interaction, idxs):
        ...
        for i in range(batch_size):
            user_id = int(batch_user[i])
            user_his_text, candidate_text, candidate_text_order, candidate_idx, candidate_text_order_description = self.get_batch_inputs(
                interaction, idxs, i, batch_user_embedding_description[i])
        ...
        if self.config['evaluation'] == 'ICL':
            try:
                batch_select_examples = np.load(os.path.join(path, 'batch_select_examples.npy'),
                                                allow_pickle=True).tolist()
            except:
                batch_select_examples = []
                query_embeddings = self.generate_embedding(list_of_item_descriptions)
                for i in tqdm(range(batch_size)):
                    user_his_descriptions = self.user_agents[int(batch_user[i])].memory_1[:-1]
        ...
        if self.config['evaluation'] != 'sequential':
            user_his_texts = None
        evaluation_prompts, messages = self.evaluation(batch_user, user_descriptions, user_his_texts,
    ...
    def evaluation(self, batch_user, user_descriptions, user_his_texts, list_of_item_descriptions,
        ...
        for i in tqdm(range(0, batch_size, self.chat_api_batch)):
            messages += asyncio.run(self.user_agents[0].llm_chat.agenerate_response_without_construction(
                evaluation_prompts[i:i + self.chat_api_batch]))

        messages = [self.rec_agent.output_parser.parse_evaluation(response["choices"][0]["message"]["content"]) for
    ...
    def parsing_output_text(self, scores, messages, idxs, candidate_texts, batch_pos_item):
        ...
        path = osp.join(self.config['record_path'], self.dataset_name, f'agentcf_ranking_result_{self.record_idx}.npy')
        np.save(path, np.array(all_recommendation_ranking_results))
